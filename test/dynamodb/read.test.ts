...
import { Committed, DynamoDBEventStoreClient, parseConfig, reduce } from "../../src/eventstore"

const { integrationTest } = JSON.parse(readFileSync(join(import.meta.dirname, 'test.json'), 'utf-8'))
const config = parseConfig(integrationTest.eventStoreConfig)

describe('read', () => {
  it('given empty key, returns zero events', async () => {
    const key = `test/${Date.now()}`
    const store = DynamoDBEventStoreClient.fromConfig(config, new DynamoDBClient({}))
